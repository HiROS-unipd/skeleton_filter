...
enum class FilterType { StateSpace, Butterworth, Undefined };

class KinematicStateFilter {
 public:
  KinematicStateFilter() {}
  KinematicStateFilter(const FilterType& filter_type,
                       const double& cutoff_frequency,
                       const double& sample_frequency = 0,
                       const unsigned int& butterworth_order = 0);
  ~KinematicStateFilter() {}

  void filter(hiros::skeletons::types::KinematicState& state,
              const double& time);

 private:
  void computePosition(hiros::skeletons::types::KinematicState& state,
                       const double& time);
  ...
  void computeOrientation(hiros::skeletons::types::KinematicState& state,
                          const double& time);
